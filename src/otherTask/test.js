...
function sostavChisla(massivChisel, chislo) {
    ...
    //         const firstEl = massivChisel[i]
    //         const secondEl = massivChisel[j]
    //         if(firstEl + secondEl === chislo && arr.filter(el => el[0] === secondEl && el[1] === firstEl).length === 0){
    //
    //             arr.push([firstEl,secondEl])
    //         }
    //     }
    // }
    // return arr
    const sortedArr = massivChisel.filter(el => el <= chislo).sort((a, b) => a - b);
    const results = [];
    const part = [];
    let sum = 0;
    ...
    for(let i = 0; i < sortedArr.length; i++) {
        ...
        if (sum > chislo) {
            ...
            if (sum === chislo) {
                results.push(part.map((el,i) => sortedArr[i]));
            }
        }
    }
